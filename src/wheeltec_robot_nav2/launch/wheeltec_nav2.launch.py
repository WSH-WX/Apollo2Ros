...
def generate_launch_description():
    ...
    bringup_cmd_group = GroupAction([
        PushRosNamespace(
            ...
            namespace=namespace),
        # 组件容器节点：将多个节点（组件）加载到同一个进程中运行。适用于需要共享资源、减少进程开销的场景
        Node(
    ...
    # Declare the launch options：运行时通过命令行传递该参数
    # 例如:ros2 launch my_package my_launch_file.launch.py map:=/path/to/another_map.yaml
