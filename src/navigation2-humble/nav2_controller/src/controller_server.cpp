  ...
  default_types_{"dwb_core::DWBLocalPlanner"}
{
  ...
  // Initialize the robot_pose subscriber
  robot_pose_subscription_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
    "robot_pose_current", 1, std::bind(&ControllerServer::robotPoseCallback, this, std::placeholders::_1));
}
...
ControllerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  plan_path_point_pub_ = create_publisher<geometry_msgs::msg::Point>("plan_path_point", 1);
...
ControllerServer::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  plan_path_point_pub_->on_activate();
...
ControllerServer::on_deactivate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  plan_path_point_pub_->on_deactivate();
...
void ControllerServer::computeControl()
{
  ...
  try {
    ...
    current_segment_index_ = 0; // 初始化
    setPlannerPathController(action_server_->get_current_goal()->path);



    for (size_t i = 0; i < path_segments_.size(); ++i) {
      const nav_msgs::msg::Path & path_self = path_segments_[i];
      RCLCPP_INFO(get_logger(), "path_self path length: %zu", path_self.poses.size());

      setPlannerPath(path_self);
      progress_checker_->reset();
      ...
      while (rclcpp::ok()) {
        ...
        //--------------------------个人添加部分-----------------------------------
        if (reachedEndOfSegment(path_self,robot_pose_) && i < path_segments_.size() - 1) {
          RCLCPP_INFO(get_logger(),
                  "Switching to next path segment!");
                  break;
        }
        if (isGoalReached() && i == path_segments_.size() - 1) {
...
void ControllerServer::setPlannerPath(const nav_msgs::msg::Path & path)
{
  ...
  current_path_ = path;
  // global_segment_path_ = path;
  
}

void ControllerServer::setPlannerPathController(const nav_msgs::msg::Path & path)
{
  // 将路径分段并存储在 path_segments_ 中
  path_segments_ = segmentPathByCusp(path);
  // 打印 path_segments_ 的长度
  RCLCPP_INFO(get_logger(), "Number of path segments: %zu", path_segments_.size());
  ...
  for (const auto &pose_stamped : path.poses) {
      ...
      // 发布 point 数据
      plan_path_point_pub_->publish(point);
    }
}
...
// -------------------------------------------------------------------------------------
std::vector<nav_msgs::msg::Path> ControllerServer::segmentPathByCusp(const nav_msgs::msg::Path & path)
{   
    std::vector<nav_msgs::msg::Path> segments;
    nav_msgs::msg::Path current_segment;

    if (path.poses.size() < 3) // 如果路径点太少，无法分段
    {
        segments.push_back(path);
        return segments;
    }

    current_segment.header = path.header;
    ...
    for (size_t i = 1; i < path.poses.size() - 1; ++i)
    {
        ...
        // 计算向量 A 和 B
        double ax = current_pose.x - prev_pose.x;
        double ay = current_pose.y - prev_pose.y;
        double bx = next_pose.x - current_pose.x;
        double by = next_pose.y - current_pose.y;

        // 尖点
        if (ax * bx + ay * by < 0) 
        {
            // 在尖点处分段
            // current_segment.poses.push_back(path.poses[i]); // 将尖点包括在当前段
            // RCLCPP_INFO(get_logger(), "current_segment path length: %zu", current_segment.poses.size());
            geometry_msgs::msg::PoseStamped last_pose = path.poses[i];
            double last_yaw = tf2::getYaw(last_pose.pose.orientation);
            // 计算扩展后的新点位置
            geometry_msgs::msg::PoseStamped new_pose = last_pose;
            new_pose.pose.position.x += dist * std::cos(last_yaw);
            new_pose.pose.position.y += dist * std::sin(last_yaw);

            // 创建一个新的路径对象，用于存储插值生成的路径点
            ...
            {
                double t = static_cast<double>(i) / num_interpolation_points;

                geometry_msgs::msg::PoseStamped interpolated_pose = last_pose;

                // 线性插值计算中间点的位置
                interpolated_pose.pose.position.x = (1 - t) * last_pose.pose.position.x + t * new_pose.pose.position.x;
                interpolated_pose.pose.position.y = (1 - t) * last_pose.pose.position.y + t * new_pose.pose.position.y;

                // 保持航向角不变
                interpolated_pose.pose.orientation = last_pose.pose.orientation;

                // 将插值点添加到原路径中
                current_segment.poses.push_back(interpolated_pose);

                // 将插值点添加到新的插值路径中
                interpolated_path.poses.push_back(interpolated_pose);
            }

            // 使用 RCLCPP_INFO 输出插值路径的长度
            RCLCPP_INFO(get_logger(), "Interpolated path length: %zu", interpolated_path.poses.size());
            RCLCPP_INFO(get_logger(), "current_segment path length-----: %zu", current_segment.poses.size());

            segments.push_back(current_segment);  // 保存当前段
            current_segment.poses.clear();  // 开始新的段
            current_segment.header = path.header; // 保持同样的头信息
            // 翻转 interpolated_path interpolated_path.poses.rbegin()
            for (auto it = std::next(interpolated_path.poses.rbegin()); it != interpolated_path.poses.rend(); ++it)
            {
              current_segment.poses.push_back(*it);
            }
            interpolated_path.poses.clear();
        }

        current_segment.poses.push_back(path.poses[i]);
    }

    // 添加最后一段路径
    current_segment.poses.push_back(path.poses.back());
    segments.push_back(current_segment);

    return segments;
}

// reachedEndOfSegment 函数，用于判断是否到达当前路径段的末尾
bool ControllerServer::reachedEndOfSegment(
  const nav_msgs::msg::Path & segment,const geometry_msgs::msg::PoseStamped & pose)
{
  // 可以根据某些条件判断，比如carrot_pose是否接近segment的最后一个点
  const auto &last_pose = segment.poses.back().pose.position;
  double distance_to_end = hypot(pose.pose.position.x - last_pose.x,
                                 pose.pose.position.y - last_pose.y);
  RCLCPP_INFO(get_logger(), "Received distance_to_end:[distance: %f]",distance_to_end);

  // // 提取路径点最后一个点的航向角（目标航向角）
  // tf2::Quaternion last_q(
  //   segment.poses.back().pose.orientation.x,
  //   segment.poses.back().pose.orientation.y,
  //   segment.poses.back().pose.orientation.z,
  //   segment.poses.back().pose.orientation.w);
  // double last_roll, last_pitch, target_yaw;
  // tf2::Matrix3x3(last_q).getRPY(last_roll, last_pitch, target_yaw);
  ...
  // // 计算航向误差
  // double heading_error = angles::shortest_angular_distance(yaw, target_yaw);
  // RCLCPP_INFO(get_logger(), "Heading error: %f radians", heading_error);

  return (distance_to_end < 0.2);//  && (fabs(heading_error) < 0.1);// 阈值
}

void ControllerServer::robotPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
  robot_pose_ = *msg;
  // RCLCPP_INFO(this->get_logger(), "Received robot pose: [x: %f, y: %f]",
