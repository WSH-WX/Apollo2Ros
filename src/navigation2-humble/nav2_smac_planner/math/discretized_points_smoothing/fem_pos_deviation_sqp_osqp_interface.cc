...
bool FemPosDeviationSqpOsqpInterface::Solve() {
  ...
  if (ref_points_.empty()) {
    RCLCPP_ERROR(rclcpp::get_logger("SmacPlannerSmoother"),
  ...
  while (pen_itr < sqp_pen_max_iter_) {
    ...
    while (sub_itr < sqp_sub_max_iter_) {
      ...
      if (!iterative_solve_res) {
        RCLCPP_ERROR(rclcpp::get_logger("SmacPlannerSmoother"),
             "Iteration at %d, solving fails with max sub iter %d",
             sub_itr, sqp_sub_max_iter_);
      ...
      if (ftol < sqp_ftol_) {
        RCLCPP_DEBUG(rclcpp::get_logger("SmacPlannerSmoother"),
             "Merit function value converges at sub itr num %d", sub_itr);

        RCLCPP_DEBUG(rclcpp::get_logger("SmacPlannerSmoother"),
             "Merit function value converges to %f, with ftol %f, under max_ftol %f",
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("SmacPlannerSmoother"),
             "ctol is %f, at pen itr %d", ctol, pen_itr);

    if (ctol < sqp_ctol_) {
      RCLCPP_DEBUG(rclcpp::get_logger("SmacPlannerSmoother"),
             "Constraint satisfied at pen itr num %d", pen_itr);

      RCLCPP_DEBUG(rclcpp::get_logger("SmacPlannerSmoother"),
             "Constraint violation value drops to %f, under max_ctol %f",
             ctol, sqp_ctol_);
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("SmacPlannerSmoother"),
             "constraint not satisfied with total itr num %d", pen_itr);

  RCLCPP_DEBUG(rclcpp::get_logger("SmacPlannerSmoother"),
             "Constraint violation value drops to %f, higher than max_ctol %f",
             ctol, sqp_ctol_);
...
bool FemPosDeviationSqpOsqpInterface::OptimizeWithOsqp(
  ...
  RCLCPP_INFO(rclcpp::get_logger("SmacPlannerSmoother"),
             "-------------------SQP-------------------------");

  if (status < 0) {
    RCLCPP_ERROR(rclcpp::get_logger("SmacPlannerSmoother"),
             "failed optimization status:\t%s", (*work)->info->status);
    return false;
  }
