  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  _plan_profile_publisher = node->create_publisher<nav_msgs::msg::Path>("osqp_plan_profile", 1);
...
void SmacPlannerHybrid::activate()
{
  ...
  _plan_profile_publisher->on_activate();
...
void SmacPlannerHybrid::deactivate()
{
  ...
  _plan_profile_publisher->on_deactivate();
  if (_costmap_downsampler) {
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  RCLCPP_INFO(
            _logger, "-----Hybrid A* -> _Smooter-----.num_iterations: %d", num_iterations);
  nav_msgs::msg::Path plan_profile;
  // Smooth plan
  if (_smoother && num_iterations > 1) {
  _smoother->smooth(plan, plan_profile, costmap, time_remaining);
  }
  RCLCPP_INFO(
            _logger, "-----Hybrid A* -> _Smooter-----end");
  _plan_profile_publisher->publish(plan_profile);
