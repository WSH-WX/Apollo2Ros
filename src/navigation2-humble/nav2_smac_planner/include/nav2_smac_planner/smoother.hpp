...
protected:
  ...
  // B样条函数：计算3阶B样条
  std::vector<Eigen::Vector2d> generateBSpline(const std::vector<Eigen::Vector2d> &control_points, int num_points);
  // 基函数计算
  double basisFunction(int i, int k, double u, const std::vector<double> &knot);
  Eigen::MatrixXd convertPathToEigenMatrix(const nav_msgs::msg::Path& path);
  ...
  bool SmoothPath(const DiscretizedPath& raw_path_points,const std::vector<double>& bounds,
                DiscretizedPath* smoothed_path_points,std::vector<std::vector<common::math::Vec2d>> point_box = {});
